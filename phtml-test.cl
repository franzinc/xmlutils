(defpackage :user (:use :test :net.html.parser))  ;; assumes phtml.cl loaded
(in-package :user)
...
(defun testit ()
  (let ((excl.test:*test-errors* 0)
	(excl.test:*test-successes* 0))
    ...
    (format t "End test: ~s,   ~d errors, ~d successes~%"
	    "parse-html" excl.test:*test-errors* excl.test:*test-successes*)
    ))