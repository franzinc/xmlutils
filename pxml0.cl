...
(funcall 'pxml-dribble-bug-hook "$Id: pxml0.cl,v 1.8 2003/05/27 19:33:22 mm Exp $")

(eval-when (compile eval)

  (defun xml-char-q (char)
  ...
  #+unused
  (defmacro xml-eql-char-q (char)
    `(eq ,char #\=))

  (defun xml-base-char-q (char)
    (declare (optimize (speed 3) (safety 1)))
    (let ((code (char-code char)))
  ...
  (defun xml-ideographic-q (char)
    (declare (optimize (speed 3) (safety 1)))
    (let ((code (char-code char)))
      (or (<= #x4E00 code #x9FA5) (= code #x3007) (<= #x3021 code #x3029))))

  (defun xml-combining-char-q (char)
    (declare (optimize (speed 3) (safety 1)))
    (let ((code (char-code char)))
  ...
  (defun xml-name-char-q (char)
    (or (xml-letter-q char) (xml-digit-q char) (eq char #\.)
	(eq char #\-) (eq char #\_) (eq char #\:)
	(xml-combining-char-q char) (xml-extender-q char)))

  (defun xml-name-start-char-q (char)
    (or (xml-letter-q char)
	 (eq #\_ char) (eq #\: char)
	 ))
...
(eval-when (compile eval)
  ...
  (defmacro make-table (&aux chars c)
	...
	(when (eql #x3fff (logand i #x3fff))
      ...
      (setf (elt chars i)
	    (if (xml-char-q c)
		(logior
		 (cond ((xml-space-q c) xml-space)
		       ((xml-digit-q c) xml-digit)
		       (t xml-other))
		 (if (xml-base-char-q c)       xml-base           0)
		 (if (xml-ideographic-q c)     xml-ideographic    0)
		 (if (xml-combining-char-q c)  xml-combining-char 0)
		 (if (xml-extender-q c)        xml-extender       0)
		 (if (xml-name-char-q c)       xml-name-char      0)
		 (if (xml-name-start-char-q c) xml-name-start     0)
		 )
	      0))
      ))
